...
impl File {
    ...
    /// The SFTP server-side handle string for this file, usable with the
    /// raw session for positional I/O.
    pub fn raw_handle(&self) -> &str {
        &self.handle
    }

    /// The underlying raw session. Its requests are multiplexed by request
    /// id, so callers may issue concurrent positional reads/writes with it.
    pub fn raw_session(&self) -> Arc<RawSftpSession> {
        self.session.clone()
    }
